...
class coin extends Controller
{
	...
	public function getBalance($donate)
	{
		...
		foreach ($donate as $key => $value) {
			if ($key === 'bitcoin') {
				$res        = $client->request(
					'GET', 'http://chainz.cryptoid.info/btc/api.dws?q=getreceivedbyaddress&a=' . $value
				);
				$contentCMC = (float)$res->getBody()->getContents();
				$total      = number_format($contentCMC, '8', '.', '');
			} else {
	...
	public function ComingSoonCoinList()
	{
		...
		$coin['notes'] = 'LaunchDate 08-01-2017';
		$coins[$i]     = $coin;
		$i++;
		$coin          = [];
		$coin['name']  = 'Linda';
		...
		$coin['logo']  = 'https://files.coinmarketcap.com/static/img/coins/32x32/linda.png';
		$coin['notes'] = 'LaunchDate 08-02-2017';
		$coins[$i]     = $coin;
		$i++;
		$coin          = [];
		$coin['name']  = 'Braincoin';
		...
		$coin['notes'] = 'Blockchain Locking up - Coin Dev Support Required';
		$coins[$i]     = $coin;
		$i++;
		$coin          = [];
		$coin['name']  = 'Coinonat';
